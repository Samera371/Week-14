// Data for the movies
export const defaultMovies = [
  {
    id: 1, // Unique identifier for the movie
    type: "movie", // Type of media (movie or TV series)
    title: "Dune", // Title of the movie
    description:
      "Paul Atreides, a brilliant and gifted young man born into a great destiny beyond his understanding, must travel to the most dangerous planet in the universe to ensure the future of his family and his people. As malevolent forces explode into conflict over the planet's exclusive supply of the most precious resource in existence, only those who can conquer their own fear will survive.", // Brief description of the movie's plot
    year: "2021", // Release year of the movie
    genre: ["Science Fiction", "Adventure"], // Genres associated with the movie
    ratings: [10, 8, 8], // Array of ratings for the movie
    imgUrl:
      "https://m.media-amazon.com/images/M/MV5BMDQ0NjgyN2YtNWViNS00YjA3LTkxNDktYzFkZTExZGMxZDkxXkEyXkFqcGdeQXVyODE5NzE3OTE@._V1_FMjpg_UX1000_.jpg", // URL to the movie's poster image
    trailerUrl: "https://www.youtube.com/watch?v=n9xhJrPXop4", // URL to the movie's trailer
  },
    {
      id: 2,
      type: "tv series",
      title: "Fallout",
      description:
        "In a future, post-apocalyptic Los Angeles brought about by nuclear decimation, citizens must live in underground bunkers to protect themselves from radiation, mutants and bandits.",
      year: "2024",
      genre: ["Science Fiction", "Action", "Adventure"],
      ratings: [10, 10, 8],
      imgUrl:
        "https://m.media-amazon.com/images/M/MV5BZjQ0YjAyNWQtMjRjMC00NzMxLTlkNjEtYWQzNmQwNGRlMGJkXkEyXkFqcGdeQXVyMDM2NDM2MQ@@._V1_.jpg",
      trailerUrl: "https://www.youtube.com/watch?v=0kQ8i2FpRDk",
    },
    {
      id: 3,
      type: "movie",
      title: "Kingdom of the Planet of the Apes",
      description:
        "Many years after the reign of Caesar, a young ape goes on a journey that will lead him to question everything he's been taught about the past and make choices that will define a future for apes and humans alike.",
      year: "2024",
      genre: ["Science Fiction", "Action", "Adventure"],
      ratings: [7, 8, 8],
      imgUrl:
        "https://m.media-amazon.com/images/M/MV5BZGI4NTEwNTAtZDcwMi00MDkxLTg1OGYtNTZmMzE3ZDljNzVlXkEyXkFqcGdeQXVyMTEyMjM2NDc2._V1_.jpg",
      trailerUrl: "https://www.youtube.com/watch?v=Kdr5oedn7q8",
    },
    {
      id: 4,
      type: "movie",
      title: "Challengers",
      description:
        "Tashi, a former tennis prodigy turned coach, turned her husband into a champion. But to overcome a losing streak, he needs to face his ex-best friend and Tashi's ex-boyfriend.",
      year: "2024",
      genre: ["Drama", "Sport"],
      ratings: [7, 8, 8],
      imgUrl:
        "https://m.media-amazon.com/images/M/MV5BNTk1MTdmYzctNzlkOS00ZTNmLWI4ZTMtMzc3OWFkMDI0MTJkXkEyXkFqcGdeQXVyMTkxNjUyNQ@@._V1_.jpg",
      trailerUrl: "https://www.youtube.com/watch?v=VobTTbg-te0",
    },
    {
      id: 5,
      type: "tv series",
      title: "Shōgun",
      description:
        "When a mysterious European ship is found marooned in a nearby fishing village, Lord Yoshii Toranaga discovers secrets that could tip the scales of power and devastate his enemies.",
      year: "2024",
      genre: ["Adventure", "Drama"],
      ratings: [10, 5, 8],
      imgUrl: "https://i.imgur.com/Wu7TP8q.jpg",
      trailerUrl: "https://www.youtube.com/watch?v=HIs9x49DK7I",
    },
    {
      id: 6,
      type: "tv series",
      title: "The Lord of the Rings: The Rings of Power",
      description:
        "Epic drama set thousands of years before the events of J.R.R. Tolkien's 'The Hobbit' and 'The Lord of the Rings' follows an ensemble cast of characters, both familiar and new, as they confront the long-feared re-emergence of evil to Middle-earth.",
      year: "2024",
      genre: ["Action", "Adventure", "Drama", "Fantasy"],
      ratings: [8, 8, 8],
      imgUrl:
        "https://m.media-amazon.com/images/M/MV5BYTZmMzJhOTktNGJjOC00NDE0LThjNTItODQ0YjBjNzg2OWQ4XkEyXkFqcGdeQXVyMDM2NDM2MQ@@._V1_.jpg",
      trailerUrl: "https://www.youtube.com/watch?v=TCwmXY_f-e0",
    },
    {
      id: 7,
      type: "movie",
      title: "IF",
      description:
        "A young girl who goes through a difficult experience begins to see everyone's imaginary friends who have been left behind as their real-life friends have grown up.",
      year: "2024",
      genre: ["Animation", "Comedy", "Drama", "Fantasy"],
      ratings: [5, 8, 7],
      imgUrl:
        "https://m.media-amazon.com/images/M/MV5BNTk3NTEwOWItOWIxYy00YzMwLTg3MWQtNmU5MmYwMTMwZTJkXkEyXkFqcGdeQXVyMTY3ODkyNDkz._V1_.jpg",
      trailerUrl: "https://www.youtube.com/watch?v=ss2KvK-w6w8",
    },
    {
      id: 8,
      type: "movie",
      title: "Bad Boys: Ride or Die",
      description:
        "This Summer, the world's favorite Bad Boys are back with their iconic mix of edge-of-your seat action and outrageous comedy but this time with a twist: Miami's finest are now on the run.",
      year: "2024",
      genre: ["Action", "Adventure", "Comedy"],
      ratings: [9, 8, 6],
      imgUrl: "https://i.redd.it/vdj798khovqc1.jpeg",
      trailerUrl: "https://www.youtube.com/watch?v=ss2KvK-w6w8",
    },
  ];